...
handle_cast({move, Direction}, #state{player=Player, location=Location} = State) ->
  ...
  RoomId = simmoa_world:get_location_id(Location),  
  NewRoomId = simmoa_world:get_location_id(NewLocation),
  simmoa_room:leave(RoomId, Avatar),
  simmoa_room:enter(NewRoomId, Avatar),
  NewState = State#state{location=NewLocation},
  simmoa_interpreter:notify({location, NewLocation}, Player),
  {noreply, NewState};

handle_cast({notify, Update}, #state{player=Player} = State) ->
  %io:format("notifying avatar of non list update ~p\n", [Update]),
  simmoa_interpreter:notify(Update, Player),
  {noreply, State};
